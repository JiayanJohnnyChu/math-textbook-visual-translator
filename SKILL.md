...
6. After each wave, assign every chapter to a reviewer other than its original translator. The reviewer reopens every 200 dpi source page and compares it with the chapter TeX and the complete compiled Chinese chapter pages. This is the only dedicated post-translation visual QA pass; the translator's separate QA is TeX-only.
...
10. Run `scripts/check_page_coverage.py` over the final source files. Every physical source page, including intentional blanks and separators, must have a source-page marker and independent visual-review coverage.

## Review boundary

The translator views every physical source page while transcribing, then performs a TeX-only QA pass over the completed chapter. Do not require the translator to reopen every page image solely for a second visual check. The independent reviewer performs the single exhaustive visual QA pass after translation, reopening every source page and checking it against the TeX and compiled Chinese output. Final aesthetic inspection may sample representative output pages, but pages containing figures, author footnotes, translator errata, dense displayed mathematics, bibliography, symbol lists, or index boundaries remain high-risk and should be checked after relevant global changes.
...
- `GLOSSARY.md`;
- `ERRATA.md`;
- one concise TeX-only translation QA record and one page-by-page independent visual-review QA table per chapter or coherent unit;
